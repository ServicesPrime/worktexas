...
const form = useForm({ ...props.zipcode });

const saveForm = () => {
    form.put(route(`${props.routeName}update`, props.zipcode.id));
};
...
const deleteForm = () => {
    Swal.fire({
        ...
        if (res.isConfirmed) {
            form.delete(route(`${props.routeName}destroy`, props.zipcode.id));
        }
    });
};
</script>
        ...
        <CardBox form @submit.prevent="saveForm">
            ...
            <template #footer>
                <BaseButtons>
                    ...
                    <BaseButton color="danger" @click="deleteForm" label="Delete" />
                </BaseButtons>
            </template>
        </CardBox>
    </LayoutMain>
